...
public class HRActionNotice
{
	...
	public static List<KeyNamePair> getEmployeeValid(MHRProcess process)
	{
		...
		String sql = MRole.getDefault().addAccessSQL(
				...
				" AND hrpe.ad_org_id = (SELECT ad_org_id FROM hr_process WHERE hr_process_ID = "+process.getHR_Process_ID() + ")";
		        ///fercho
	...
	public ArrayList<ValueNamePair> getConcept(MHRProcess process, boolean isFieldProcessNull)
	{
		...
		try
		{
			...
			while (rs.next())
			{
				vp = new ValueNamePair(rs.getString(1), rs.getString(3) + "_" + rs.getString(2));
				data.add(vp);
			}
			rs.close();
			pstmt.close();
