...
public class HRActionNotice
{
	...
	{	
		StringBuilder sqlQuery = new StringBuilder("SELECT DISTINCT o.Name,hp.Name,"   // AD_Org_ID, HR_Process_ID -- 1,2
	...
	public static KeyNamePair[] getProcess()
	{
		...
		StringBuilder sql2 = new StringBuilder(sql).append(" AND hrp.IsActive = 'Y' ");
		sql2.append(" ORDER BY hrp.DocumentNo, hrp.Name");

		return DB.getKeyNamePairs(sql2.toString(), true);
	} //getProcess
	...
	public static List<KeyNamePair> getEmployeeValid(MHRProcess process)
	{
		...
		StringBuilder sql2 = new StringBuilder(sql).append(" AND hrpe.IsActive = 'Y' ");
		if ( process.getHR_Payroll_ID() != 0){
			sql2.append(" AND (hrpe.HR_Payroll_ID =" ).append(process.getHR_Payroll_ID()).append( " OR hrpe.HR_Payroll_ID is NULL)");
			if ( process.getHR_Department_ID() > 0 )
 				sql2.append(" AND (hrpe.HR_Department_ID =" ).append(process.getHR_Department_ID()).append(" OR hrpe.HR_Department_ID is NULL)");
 			if ( process.getHR_Employee_ID() > 0 )
 				sql2.append(" AND (hrpe.HR_Employee_ID =" ).append( process.getHR_Employee_ID()).append(" OR hrpe.HR_Employee_ID is NULL)");
		}
		sql2.append(" ORDER BY bp.Name ");
		PreparedStatement pstmt = null;
		ResultSet rs = null;

		try{
			pstmt = DB.prepareStatement(sql2.toString(), null);
			rs = pstmt.executeQuery();
			while (rs.next())
			{
	...
	public ArrayList<ValueNamePair> getConcept(MHRProcess process, boolean isFieldProcessNull)
	{
		...
		StringBuilder sql = new StringBuilder("SELECT DISTINCT hrpc.HR_Concept_ID,hrpc.Name,hrpc.Value ")
								.append( " FROM HR_Concept hrpc ")
								.append( " INNER JOIN HR_Attribute hrpa ON (hrpa.HR_Concept_ID=hrpc.HR_Concept_ID)")
								.append( " WHERE hrpc.AD_Client_ID = " ).append(Env.getAD_Client_ID(Env.getCtx()))
								.append( " AND hrpc.IsActive = 'Y' AND hrpc.IsRegistered = 'Y' AND hrpc.Type != 'E'")
								.append( " AND (hrpa.HR_Payroll_ID = " ).append(m_HR_Payroll_ID).append( " OR hrpa.HR_Payroll_ID IS NULL)");
		if (!isFieldProcessNull)    
		{ // Process
			if (process.getHR_Payroll_ID() != 0) // Process & Payroll
 				sql.append( " AND (hrpa.HR_Payroll_ID = " ).append(process.getHR_Payroll_ID()).append( " OR hrpa.HR_Payroll_ID is NULL)") ;
 			if (process.getHR_Department_ID() != 0 ); // Process & Department
 				sql.append( " AND (hrpa.HR_Department_ID = " ).append(process.getHR_Department_ID()).append(" OR hrpa.HR_Department_ID is NULL)") ;
 			if (process.getHR_Department_ID() != 0 ); // Process & Employee
 				sql.append( " AND (hrpa.HR_Employee_ID = " ).append( process.getHR_Employee_ID()).append(" OR hrpa.HR_Employee_ID is NULL)") ;	
		}
		sql.append(" ORDER BY hrpc.Name");
	...
	public int seekMovement(Timestamp dt)
	{
		...
		StringBuilder sql = new StringBuilder("SELECT HR_Movement_ID FROM HR_Movement WHERE HR_Process_ID = ").append(Process_ID)
								.append(" AND C_BPartner_ID =").append(Employee_ID).append(" AND HR_Concept_ID = ").append(Concept_ID)
								.append(" AND TRUNC(ValidFrom) = TRUNC(").append(date).append(")");
		if ( (Process_ID+Employee_ID+Concept_ID) > 0 ){
			HR_Movement_ID = DB.getSQLValue(null,sql.toString());
		}
	...
	public static  String getSQL_ColumnType(Properties ctx,String sqlValue) {
		...
		if (Env.isBaseLanguage(Env.getCtx(), I_AD_Ref_List.Table_Name)) {
			sql.append("SELECT zz.Name FROM AD_Ref_List zz WHERE zz.AD_Reference_ID=").append(columnType_Ref_ID); 
		}
		else {
			sql.append("SELECT zz.Name FROM AD_Ref_List zz, AD_Ref_List_Trl zzt")
				.append(" WHERE zz.AD_Reference_ID=").append(columnType_Ref_ID)
				.append(" AND zzt.AD_Ref_List_ID=zz.AD_Ref_List_ID")
				.append(" AND zzt.AD_Language=").append(DB.TO_STRING(Env.getAD_Language(Env.getCtx())));
		}
		sql.append(" AND zz.Value = ").append(sqlValue);
		return sql.toString();
	} // getSQL_ColumnType
