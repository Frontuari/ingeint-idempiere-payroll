...
public class MHRConcept extends X_HR_Concept
{
	...
	{
		StringBuilder sql = new StringBuilder("HR_Expense_Acct FROM HR_Concept c ")
		.append("INNER JOIN HR_Concept_Acct ca ON (c.HR_Concept_ID=ca.HR_Concept_ID) ")
				.append("WHERE c.HR_Concept_ID =").append(getHR_Concept_ID());
		int result = DB.getSQLValue("ConceptCR", sql.toString());
		if (result > 0)
			return result;
		return 0;
	}
	...
	{
		StringBuilder sql = new StringBuilder("HR_Revenue_Acct FROM HR_Concept c ")
		.append("INNER JOIN HR_Concept_Acct ca ON (c.HR_Concept_ID=ca.HR_Concept_ID) ")
				.append("WHERE c.HR_Concept_ID =").append(getHR_Concept_ID());
