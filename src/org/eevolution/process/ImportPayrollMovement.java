...
public class ImportPayrollMovement extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		...
		sql = new StringBuffer ("UPDATE I_HR_Movement "
			...
			+ " AD_Org_ID = COALESCE ((SELECT AD_Org_ID FROM HR_Employee e"
			+ " WHERE I_HR_Movement.c_bpartner_id=e.c_bpartner_id AND I_HR_Movement.AD_Client_ID=e.AD_Client_ID),0), "
		...
		try
		{
			...
			while (rs.next())
			{
				...
				if (newMovement)			//	Insert new HR Movement
				{
					...
					// re-write some data based on concept in case they have changed
					hrmovement.setHR_Concept_Category_ID(hrconcept.getHR_Concept_Category_ID());
					hrmovement.setColumnType(hrconcept.getColumnType());
					hrmovement.setIsRegistered(hrconcept.isRegistered());
					hrmovement.setIsPrinted(hrconcept.isPrinted());
					
					//fercho  Movement inherit the original ORG_ID and Process_ID
					hrmovement.setAD_Org_ID(imp.getAD_Org_ID());
					hrmovement.setHR_Process_ID(imp.getHR_Process_ID());
					///fercho
					if (hrmovement.save())
					{
						noUpdatehrm++;
