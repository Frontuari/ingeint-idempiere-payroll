...
public class HRActionNotice {
	...
	public static int sHR_Movement_ID = 0; // // initial not exist record in
											// Movement to actual date
	...
	public void configureMiniTable(IMiniTable miniTable) {
		...
		// Msg.translate(Env.getCtx(), "AD_Rule_ID"));
	...
	public static void executeQuery(Properties ctx, IMiniTable miniTable) {
		StringBuffer sqlQuery = new StringBuffer("SELECT DISTINCT o.Name,hp.Name," // AD_Org_ID,
																					// HR_Process_ID
																					// --
																					// 1,2
				+ " bp.Name,hc.Name,hm.ValidFrom," // HR_Employee_ID,HR_Concept_ID,ValidFrom,ColumnType
													// -- 3,4,5
				+ "("
				+ getSQL_ColumnType(ctx, "hc.ColumnType")
				...
				+ " INNER JOIN HR_Employee e ON (e.C_BPartner_ID=bp.C_BPartner_ID)" + " INNER JOIN HR_Concept hc ON (hm.HR_Concept_ID=hc.HR_Concept_ID)" + " LEFT OUTER JOIN AD_Rule er ON (hm.AD_Rule_ID=er.AD_Rule_ID)" + " WHERE hm.Processed='N' AND hp.HR_Process_ID = " + m_HR_Process_ID
				// + " AND IsStatic = 'Y' " // Just apply incidences [add
				// 30Dic2006 to see everything]
	...
	/**
	 * get Process parameter: MHRProcess
	 */
	public static KeyNamePair[] getProcess() {
		String sql = MRole.getDefault().addAccessSQL("SELECT hrp.HR_Process_ID,hrp.DocumentNo ||'-'|| hrp.Name,hrp.DocumentNo,hrp.Name FROM HR_Process hrp", "hrp", MRole.SQL_FULLYQUALIFIED, MRole.SQL_RO) + " AND hrp.IsActive = 'Y' " + " AND hrp.DocStatus IN ('DR', 'PR', 'IP') ";
	...
	/**
	 * get Employee to Valid Payroll-Department-Employee of Process Actual
	...
	public static List<KeyNamePair> getEmployeeValid(MHRProcess process) {
		...
		list.add(pp);
		// String sql =
		// MRole.getDefault().addAccessSQL("SELECT DISTINCT bp.C_BPartner_ID,bp.Name FROM HR_Employee hrpe INNER JOIN C_BPartner bp ON(bp.C_BPartner_ID=hrpe.C_BPartner_ID)",
		// "hrpe", MRole.SQL_FULLYQUALIFIED, MRole.SQL_RO) +
		// " AND hrpe.IsActive = 'Y' " +
		// // /fercho getEmployeeValid just for the Employee from the Org_ID
		// " AND hrpe.ad_org_id = (SELECT ad_org_id FROM hr_process WHERE hr_process_ID = "
		// + process.getHR_Process_ID() + ")";
		// // /fercho
		// if (process.getHR_Payroll_ID() != 0) {
		// sql += " AND (hrpe.HR_Payroll_ID =" + process.getHR_Payroll_ID() +
		// " OR hrpe.HR_Payroll_ID is NULL)";
		// if (process.getHR_Department_ID() > 0)
		// sql += " AND (hrpe.HR_Department_ID =" +
		// process.getHR_Department_ID() + " OR hrpe.HR_Department_ID is NULL)";
		// if (process.getHR_Employee_ID() > 0)
		// sql += " AND (hrpe.HR_Employee_ID =" + process.getHR_Employee_ID() +
		// " OR hrpe.HR_Employee_ID is NULL)";
		// }
		// sql += " ORDER BY bp.Name ";

		String sql = MRole.getDefault().addAccessSQL("SELECT DISTINCT bp.C_BPartner_ID,bp.Name FROM HR_Employee hrpe INNER JOIN C_BPartner bp ON(bp.C_BPartner_ID=hrpe.C_BPartner_ID)", "hrpe", MRole.SQL_FULLYQUALIFIED, MRole.SQL_RO) + " AND hrpe.IsActive = 'Y' ";
		if (process.getHR_Payroll_ID() != 0) {
			sql += " AND (hrpe.HR_Payroll_ID =" + process.getHR_Payroll_ID() + " OR hrpe.HR_Payroll_ID is NULL)";
			/*
			 * if ( process.getHR_Department_ID() != 0 ); sql +=
			 * " AND (hrpe.HR_Department_ID ="
			 * +process.getHR_Department_ID()+" OR hrpe.HR_Department_ID is NULL)"
			 * ; if ( process.getHR_Employee_ID() != 0 ); sql +=
			 * " AND (hrpe.HR_Employee_ID =" +
			 * process.getHR_Employee_ID()+" OR hrpe.HR_Employee_ID is NULL)" ;
			 */
		}
		sql += " ORDER BY bp.Name ";
	...
	public ArrayList<ValueNamePair> getConcept(MHRProcess process, boolean isFieldProcessNull) {
		...
		// String sql =
		// "SELECT DISTINCT hrpc.HR_Concept_ID,hrpc.Name,hrpc.Value " +
		...
		// // + " AND (hrpa.HR_Payroll_ID = " +m_HR_Payroll_ID+
		// // " OR hrpa.HR_Payroll_ID IS NULL)"
		// ;
		// if (!isFieldProcessNull) { // Process
		// if (process.getHR_Payroll_ID() != 0) // Process & Payroll
		// sql = sql + " AND (hrpa.HR_Payroll_ID = " +
		// process.getHR_Payroll_ID() + " OR hrpa.HR_Payroll_ID is NULL)";
		// if (process.getHR_Department_ID() != 0)
		// ; // Process & Department
		// sql = sql + " AND (hrpa.HR_Department_ID = " +
		// process.getHR_Department_ID() + " OR hrpa.HR_Department_ID is NULL)";
		// if (process.getHR_Department_ID() != 0)
		// ; // Process & Employee
		// sql = sql + " AND (hrpa.HR_Employee_ID = " +
		// process.getHR_Employee_ID() + " OR hrpa.HR_Employee_ID is NULL)";
		// }
		// sql = sql + " ORDER BY hrpc.Value";

		int HR_Payroll_ID = process.getHR_Payroll_ID();

		String sql = "SELECT DISTINCT hrpc.HR_Concept_ID,hrpc.Name,hrpc.Value " + " FROM HR_Concept hrpc " + " INNER JOIN HR_Attribute hrpa ON (hrpa.HR_Concept_ID=hrpc.HR_Concept_ID)" + " WHERE hrpc.AD_Client_ID = " + Env.getAD_Client_ID(Env.getCtx())
				+ " AND hrpc.IsActive = 'Y' AND hrpc.Type = 'C'" // AND
																	// hrpc.IsRegistered
																	// = 'Y'
		...
		try {
			...
			while (rs.next()) {
				vp = new ValueNamePair(rs.getString(1), /*
														 * rs.getString(3) + "_"
														 * +
														 */rs.getString(2));
				data.add(vp);
	...
	public int seekMovement(Timestamp dt) {
		...
		if ((Process_ID + Employee_ID + Concept_ID) > 0) {
			String sql = "SELECT HR_Movement_ID  FROM HR_Movement WHERE HR_Process_ID = " + Process_ID + " AND C_BPartner_ID =" + Employee_ID + " AND HR_Concept_ID = " + Concept_ID + " AND TRUNC(ValidFrom) = TRUNC( " + date + " )";
			HR_Movement_ID = DB.getSQLValue(null, sql);
		}
	...
	public static String getSQL_ColumnType(Properties ctx, String sqlValue) {
		int columnType_Ref_ID = MTable.get(ctx, I_HR_Concept.Table_ID).getColumn(I_HR_Concept.COLUMNNAME_ColumnType).getAD_Reference_Value_ID();
		String sql;
		if (Env.isBaseLanguage(Env.getCtx(), I_AD_Ref_List.Table_Name)) {
			...
			sql = "SELECT zz.Name FROM AD_Ref_List zz, AD_Ref_List_Trl zzt" + " WHERE zz.AD_Reference_ID=" + columnType_Ref_ID + " AND zzt.AD_Ref_List_ID=zz.AD_Ref_List_ID" + " AND zzt.AD_Language=" + DB.TO_STRING(Env.getAD_Language(Env.getCtx()));
		}
