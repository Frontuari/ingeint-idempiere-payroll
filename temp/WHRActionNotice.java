 ...
 */
public class WHRActionNotice extends HRActionNotice implements IFormController, EventListener, WTableModelListener, ValueChangeListener {
...
//		row.appendChild(fieldText.getComponent());
	...
	private void configureMiniTable() {
		...
		ColumnInfo[] layout = {
				...
				new ColumnInfo(Msg.translate(Env.getCtx(), "AD_Org_ID"), "", String.class), new ColumnInfo(Msg.translate(Env.getCtx(), "HR_Concept_ID"), "", String.class), new ColumnInfo(Msg.translate(Env.getCtx(), "ValidFrom"), "", Timestamp.class),
				new ColumnInfo(Msg.translate(Env.getCtx(), "ColumnType"), "", String.class), new ColumnInfo(Msg.translate(Env.getCtx(), "Qty"), "", BigDecimal.class), new ColumnInfo(Msg.translate(Env.getCtx(), "Amount"), "", BigDecimal.class),
				new ColumnInfo(Msg.translate(Env.getCtx(), "ServiceDate"), "", Timestamp.class), new ColumnInfo(Msg.translate(Env.getCtx(), "Text"), "", String.class), new ColumnInfo(Msg.translate(Env.getCtx(), "Description"), "", String.class) };
	...
	public void onEvent(Event e) {
		...
		if (((KeyNamePair) fieldProcess.getSelectedItem().getValue()).getKey() > 0) {
			process = new MHRProcess(Env.getCtx(), ((KeyNamePair) fieldProcess.getSelectedItem().getValue()).getKey(), null);
			...
			if (e.getTarget() == fieldProcess) {
				...
				if(process.getHR_Period_ID() > 0)
				{
					...
					fieldValidFrom.setValue(period.getEndDate());
				}
				else
				{
					fieldValidFrom.setValue(process.getDateAcct());
				}
		...
		if (e.getTarget() == fieldEmployee) {
			m_C_BPartner_ID = ((KeyNamePair) fieldEmployee.getSelectedItem().getValue()).getKey();
			fieldConcept.removeAllItems();
			ArrayList<ValueNamePair> conceptData = getConcept(process, fieldProcess != null);
		...
		if (e.getTarget() == fieldConcept) {
			ValueNamePair pp = (ValueNamePair) fieldConcept.getSelectedItem().getValue();
			if (pp != null)
				try {
					m_HR_Concept_ID = Integer.parseInt(((ValueNamePair) fieldConcept.getSelectedItem().getValue()).getValue());
				} catch (Exception e1) {					
					e1.printStackTrace();
					throw new AdempiereException("@SelectRecord@");
				}
			...
			if (m_HR_Concept_ID > 0) {
				...
				sHR_Movement_ID = seekMovement((Timestamp) fieldValidFrom.getValue());
				...
				if (concept.getColumnType().equals(X_HR_Concept.COLUMNTYPE_Quantity)) { // Concept
																						// Type
		...
		if (e.getTarget() == bOk) {
			...
			if (m_HR_Concept_ID <= 0 || fieldProcess.getSelectedItem().getValue() == null || ((KeyNamePair) fieldProcess.getSelectedItem().getValue()).getKey() <= 0 || fieldEmployee.getSelectedItem().getValue() == null || ((KeyNamePair) fieldEmployee.getSelectedItem().getValue()).getKey() <= 0) { // required
																																																																											// fields
			} else {
				saveMovement();
			}
	...
	public void saveMovement() {
		MHRProcess process = new MHRProcess(Env.getCtx(), ((KeyNamePair) fieldProcess.getSelectedItem().getValue()).getKey(), null);
		...
		movementOK.setAD_Org_ID(process.getAD_Org_ID());
		movementOK.setDescription(fieldDescription.getValue() != null ? (String) fieldDescription.getValue().toString() : "");
		movementOK.setHR_Process_ID(((KeyNamePair) fieldProcess.getSelectedItem().getValue()).getKey());
		movementOK.setC_BPartner_ID(((KeyNamePair) fieldEmployee.getSelectedItem().getValue()).getKey());
		movementOK.setHR_Concept_ID(Integer.parseInt(((ValueNamePair) fieldConcept.getSelectedItem().getValue()).getValue()));
		...
		if ((movementOK.getAmount() == null || Env.ZERO.compareTo(movementOK.getAmount()) == 0) && (movementOK.getQty() == null || Env.ZERO.compareTo(movementOK.getQty()) == 0) && (movementOK.getServiceDate() == null)
	...
	private void executeQuery() {
		...
		try {
			...
			while (rs.next()) {
				...
				miniTable.setValueAt(rs.getObject(7) != null ? rs.getBigDecimal(7) : Env.ZERO, row, 4); // Qty
				miniTable.setValueAt(rs.getObject(8) != null ? rs.getBigDecimal(8) : Env.ZERO, row, 5); // Amount
